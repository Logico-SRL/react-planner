    ...
    if (typeof window != 'undefined') window.addEventListener('keydown', function (event) {
      ...
      switch (event.keyCode) {
        ...
        case _constants.KEYBOARD_BUTTON_CODE.DELETE:
          {
            if ([_constants.MODE_IDLE].includes(mode)) store.dispatch((0, _projectActions.remove)());
            break;
          }
        ...
        case _constants.KEYBOARD_BUTTON_CODE.C:
          {
            ...
            if (mode === _constants.MODE_IDLE && (selected.holes.size || selected.areas.size || selected.items.size || selected.lines.size)) {
